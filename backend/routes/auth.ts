...
import { z } from "zod";
import { createUser, findUserByEmail } from "../lib/mongo";
...
router.post("/signup", async (req, res) => {
  try {
    ...
    const token = signJwt(
      { sub: userId.toHexString(), email: email.toLowerCase(), name: name.trim() },
      { expiresIn: "7d" },
    );
    res.status(201).json({ token, user: { id: userId.toHexString(), name: name.trim(), email: email.toLowerCase() } });
  } catch (err: any) {
...
router.post("/login", async (req, res) => {
  try {
    ...
    const token = signJwt(
      { sub: user._id.toHexString(), email: user.email, name: user.name },
      { expiresIn: "7d" },
    );
    res.json({ token, user: { id: user._id.toHexString(), name: user.name, email: user.email } });
  } catch (err: any) {
