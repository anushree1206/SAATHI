...
type DashboardStats = { totalConversations: number; totalMessages: number; averageAssistantResponseLength: number; lastActiveAt: string | null };
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([dashboardData, conversationList]) => { setStats(dashboardData); setConversations(conversationList); })
      .catch((error: any) => toast({ title: "Unable to load dashboard", description: error?.data?.error ?? error?.message ?? "Try refreshing.", variant: "destructive" }))
  ...
  if (!user) {
    return (
      <div className="max-w-3xl mx-auto p-6 bg-card/80 border border-border rounded-3xl shadow-xl mt-10 text-center">
        ...
        <Link href="/login"><Button className="px-8">Sign in</Button></Link>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-5xl mx-auto p-6">
      <div className="mb-8 space-y-3">
        ...
        <p className="text-muted-foreground max-w-2xl">Welcome back, {user.name}. Track your chat activity and continue where you left off.</p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-4 mb-8">
        {[
          ...
          { label: "Avg. AI response", value: stats ? `${stats.averageAssistantResponseLength} chars` : "—" },
          { label: "Last active", value: stats?.lastActiveAt ? new Date(stats.lastActiveAt).toLocaleString() : "—", small: true },
        ].map(({ label, value, small }) => (
          <div key={label} className="rounded-3xl border border-border bg-card/80 p-5">
            <p className="text-sm text-muted-foreground">{label}</p>
            <p className={`mt-3 font-semibold text-white ${small ? "text-base" : "text-3xl"}`}>{value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="rounded-3xl border border-border bg-card/80 p-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Click any item to copy the conversation ID.</p>
          </div>
          <Link href="/chat"><Button className="bg-primary text-primary-foreground">Start a new chat</Button></Link>
        </div>
