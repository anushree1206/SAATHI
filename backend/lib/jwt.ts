...
export function signJwt(payload: Record<string, unknown>, options?: { expiresIn?: string | number }) {
  ...
  const body: Record<string, unknown> = { ...payload, iat: now };
  if (options?.expiresIn) body.exp = now + parseExpires(options.expiresIn);
  ...
  return `${encodedHeader}.${encodedBody}.${sign(`${encodedHeader}.${encodedBody}`)}`;
}
...
export function verifyJwt(token: string): Record<string, unknown> {
  ...
  const payload = JSON.parse(base64UrlDecode(encodedBody).toString("utf8")) as Record<string, unknown>;
