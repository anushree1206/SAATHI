    ...
    <nav className="w-full border-b border-border/40 backdrop-blur-md bg-background/80 sticky top-0 z-50">
      <div className="max-w-6xl mx-auto px-4 md:px-8 h-16 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-8">
          ...
          <Link href="/chat" className={`text-sm font-medium transition-colors hover:text-primary ${location === "/chat" ? "text-primary" : "text-muted-foreground"}`} data-testid="link-nav-demo">Chat</Link>
          {user && (
        ...
        <div className="flex items-center gap-4">
          {user ? (
            <>
              ...
              <Button onClick={signOut} variant="secondary" className="font-semibold rounded-full px-4">Logout</Button>
            </>
          ) : (
            <>
              <Link href="/login">
                <Button variant="secondary" className="font-semibold rounded-full px-4">Login</Button>
              </Link>
              <Link href="/chat" data-testid="link-nav-try">
